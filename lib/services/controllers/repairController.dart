...
class RepairController {
  Future<List<RepairModel>> getApi() async {
    const String apiUrl = 'http://171.244.8.103:9003/api/bridgeRepair?keyword=&status=0&startSuaChua=&endSuaChua=&bridgeId=0';
  ...
  Future<List<RepairModel>> getApiWithId(int id) async {
    final String apiUrl = 'http://171.244.8.103:9003/api/bridgeRepair?keyword=&status=0&startSuaChua=&endSuaChua=&bridgeId=$id';
    http.Response response = await http.get(Uri.parse(apiUrl));
    if (response.statusCode == 200) {
      var jsonResponse = json.decode(response.body);
      var data = jsonResponse["Data"];
